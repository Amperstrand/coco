import { A as AuthSessionError, B as OperationInProgressError, C as assertUnitAmount, D as normalizeUnitList, E as normalizeUnitAmount, F as MintFetchError, G as TokenValidationError, H as ProofOperationError, I as MintOperationError, J as UnknownMintError, K as UnitMismatchError, L as MintQuoteKeyError, M as DerivationIndexExhaustedError, N as HttpResponseError, O as parseUnitAmount, P as KeysetSyncError, R as MintQuoteValidationError, S as assertSameUnit, T as normalizeUnit, U as ProofValidationError, V as PaymentRequestError, W as QuoteIdentityConflictError, _ as serializeOutputData, b as toAmount, d as getSecretsFromSerializedOutputData, f as mapProofToCoreProof, j as AuthSessionExpiredError, k as sameUnitAmount, l as generateSubId, n as buildYHexMapsForSecrets, p as normalizeMintUrl, q as UnitValidationError, r as computeYHexForSecrets, s as deserializeOutputData, t as assertNonNegativeInteger, u as getProofStateInputsFromSerializedOutputs, w as isUnitAmountLikeObject, x as DEFAULT_UNIT, y as sumAmounts, z as NetworkError } from "./utils-CmANTq2P.js";
import { a as operationHistoryId, c as projectMeltOperation, d as projectReceiveOperation, f as projectSendOperation, i as legacyHistoryId, l as projectMintOperation, n as isLegacyHistoryEntry, o as parseHistoryEntryId, r as isOperationHistoryEntry, s as projectLegacyHistoryRow, t as compareHistoryEntries, u as projectOperationToHistoryEntry } from "./History-DUeO5ZVe.js";
import { f as meltQuoteFromBolt11Response, g as resolveOnchainMeltFeeOption, h as meltQuoteToMethodSnapshot, m as meltQuoteFromOnchainResponse, p as meltQuoteFromBolt12Response, t as MeltOperationService } from "./MeltOperationService-CmEbNCeA.js";
import { a as getMintQuoteRemoteState, c as mintQuoteFromBolt11Response, d as mintQuoteToMethodSnapshot, f as assessMintQuoteClaimability, h as mintQuoteObservationFromOnchainResponse, i as getMintQuoteAvailableAmount, l as mintQuoteFromBolt12Response, m as mintQuoteObservationFromBolt12Response, n as deriveBolt11MintQuoteState, o as isMintQuotePending, p as mintQuoteObservationFromBolt11Response, r as getMintQuoteAmount, s as isStatefulMintQuote, t as applyBolt11MintQuoteStateFallback, u as mintQuoteFromOnchainResponse } from "./MintQuote-DE-THtlv.js";
...
var KeyRingService = class KeyRingService {
	...
	async generateKeyPairForPurpose(purpose, options) {
		...
		const keyPair = await this.keyRingRepository.deriveAndPersistKeyPair(purpose, (derivationIndex) => {
			...
			return {
				publicKeyHex: purpose === "nut20_mint_quote" ? this.getCompressedPublicKeyHex(secretKey) : this.getPublicKeyHex(secretKey),
				secretKey
			};
		});
...
var ProofService = class {
	...
	logger;
	outputDataCreator;
	constructor(counterService, proofRepository, walletService, mintService, keyRingService, seedService, logger, eventBus, outputDataCreator) {
...
var DefaultSendHandler = class {
	...
	*/
	async prepare(ctx) {
		...
		const { forceSwap = false } = operation.methodData;
		const exactProofs = forceSwap ? [] : await proofService.selectProofsToSend(mintUrl, {
			amount,
			unit
		}, false);
	...
	*/
	async recoverExecuting(ctx) {
		...
		const proofInputs = await proofRepository.getProofsBySecrets(operation.mintUrl, operation.inputProofSecrets);
		if (proofInputs.length !== operation.inputProofSecrets.length) throw new ProofValidationError("Cannot recover send operation: missing input proof metadata");
		let inputStates;
		try {
...
var P2pkSendHandler = class {
	...
	*/
	async prepare(ctx) {
		...
		const { mintUrl, amount, unit } = operation;
		const p2pkOptions = this.getP2pkOptions(operation.methodData);
	...
	*/
	async recoverExecuting(ctx) {
		const { operation, wallet, proofRepository, proofService, logger } = ctx;
		const proofInputs = await proofRepository.getProofsBySecrets(operation.mintUrl, operation.inputProofSecrets);
		if (proofInputs.length !== operation.inputProofSecrets.length) throw new ProofValidationError("Cannot recover P2PK send operation: missing input proof metadata");
		let inputStates;
		try {
			inputStates = await wallet.checkProofsStates(proofInputs);
		} catch (error) {
		...
		if (sendProofs.length > 0) token = {
			...
			if (!(await wallet.checkProofsStates(getProofStateInputsFromSerializedOutputs(operation.outputData.send))).every((state) => state.state === "SPENT")) throw new Error("Recovered P2PK swap succeeded but token could not be reconstructed");
		}
		const pending = {
			...operation,
			state: "pending",
...
var MintQuoteApi = class {
	...
	async create(input) {
		...
		if (input.method === "onchain") return this.quoteLifecycle.createMintQuote(input.mintUrl, input.method, { unit: normalizeUnit(input.unit, { defaultUnit: DEFAULT_UNIT }) });
		const custom = input;
		const createQuoteData = {
			amount: parseUnitAmount(custom.amount, { explicitUnit: custom.unit }),
			...custom.description !== void 0 ? { description: custom.description } : {},
			...custom.locked === true ? { locked: true } : {}
		};
		return this.quoteLifecycle.createMintQuote(custom.mintUrl, custom.method, createQuoteData);
	}
	get(input) {
...
var QuoteLifecycle = class {
	...
	*/
	...
	mintQuoteFromSnapshot(mintUrl, method, quote, validateAccounting) {
		...
		const statefulSnapshot = quote;
		const rawAmount = statefulSnapshot.amount;
		if (rawAmount === void 0 || rawAmount === null) throw new MintQuoteValidationError("Mint quote " + String(statefulSnapshot.quote) + " has invalid amount");
		const customAmount = Amount$1.from(rawAmount);
		if (customAmount.isZero()) throw new MintQuoteValidationError("Mint quote " + String(statefulSnapshot.quote) + " has invalid amount");
		const customResponse = {
			...statefulSnapshot,
			amount: customAmount
		};
		return {
			...validateAccounting ? mintQuoteFromBolt11Response(mintUrl, customResponse) : mintQuoteObservationFromBolt11Response(mintUrl, customResponse),
			method
		};
	}
...
var Manager = class {
	...
	*/
	registerMintMethod(method, handler) {
	...
	* operation services resolve handlers per call.
	*/
	registerMeltMethod(method, handler) {
	...
	buildCoreServices(repositories, seedGetter) {
		...
		const proofService = new ProofService(counterService, repositories.proofRepository, walletService, mintService, keyRingService, seedService, proofLogger, this.eventBus, this.outputDataCreator);
...
var MemoryKeyRingRepository = class {
	...
	async deriveAndPersistKeyPair(purpose, derive) {
		...
		for (const keypair of this.keyPairs.values()) {
			...
			if (keypair.derivationIndex != null && keypair.derivationIndex > greatestStoredIndex) greatestStoredIndex = keypair.derivationIndex;
		}
