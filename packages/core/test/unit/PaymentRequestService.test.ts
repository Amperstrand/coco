...
describe('PaymentRequestService', () => {
  ...
  const createResolvedRequest = (
    ...
    return {
      paymentRequest: new PaymentRequest({ transport: paymentRequestTransport, id: 'test-id', amount: options.amount, unit: options.unit ?? 'sat', mints: allowedMints }),
  ...
  const createP2pkResolvedRequest = (
    ...
    return {
      ...resolved,
      paymentRequest: new PaymentRequest({ transport: resolved.paymentRequest.transport, id: resolved.paymentRequest.id, amount: resolved.amount, unit: resolved.unit, mints: resolved.allowedMints, description: resolved.paymentRequest.description, singleUse: resolved.paymentRequest.singleUse, nut10: p2pkNut10() }),
      spendingCondition: {
        kind: 'P2PK',
        p2pk: {
          kind: 'P2PK',
  ...
  describe('parse', () => {
    it('should decode an inband payment request', async () => {
      const pr = new PaymentRequest({ transport: [], id: 'request-id-1', amount: 100, unit: 'sat', mints: [testMintUrl], description: 'Test payment' });
    ...
    it('should decode an HTTP POST payment request', async () => {
      const pr = new PaymentRequest({ transport: [{ type: PaymentRequestTransportType.POST, target: testHttpTarget }], id: 'request-id-2', amount: 200, unit: 'sat', mints: [testMintUrl, testMintUrl2], description: 'HTTP payment' });
      const encoded = pr.toEncodedRequest();
    ...
    it('should decode a payment request without amount', async () => {
      const pr = new PaymentRequest({ transport: [], id: 'request-id-3', unit: 'sat', mints: [testMintUrl] });
      const encoded = pr.toEncodedRequest();
    ...
    it('should expose a normalized P2PK spending condition requirement', async () => {
      const pr = new PaymentRequest({ transport: [], id: 'request-id-p2pk', amount: 100, unit: 'sat', mints: [testMintUrl], description: 'P2PK payment', singleUse: false, nut10: p2pkNut10() });
      const encoded = pr.toEncodedRequest();
    ...
    it('filters P2PK payable mints to trusted sufficient allowed mints with NUT-11 support', async () => {
      const pr = new PaymentRequest({ transport: [], id: 'request-id-p2pk-filter', amount: 100, unit: 'sat', mints: [testMintUrl, testMintUrl2], singleUse: false, nut10: p2pkNut10() });
      const encoded = pr.toEncodedRequest();
    ...
    it('excludes individual P2PK payable mints when their NUT-11 lookup fails', async () => {
      ...
      const pr = new PaymentRequest({ transport: [], id: 'request-id-p2pk-filter-offline-mint', amount: 100, unit: 'sat', mints: [testMintUrl, testMintUrl2], singleUse: false, nut10: p2pkNut10() });
      const encoded = pr.toEncodedRequest();
    ...
    it('returns no payable mints for P2PK requests when no matching mint advertises NUT-11', async () => {
      ...
      const pr = new PaymentRequest({ transport: [], id: 'request-id-p2pk-no-mints', amount: 100, unit: 'sat', mints: [testMintUrl, testMintUrl2], singleUse: false, nut10: p2pkNut10() });
      const encoded = pr.toEncodedRequest();
    ...
    it('keeps unsupported NUT-10 requirements as diagnostics and returns no payable mints', async () => {
      ...
      const pr = new PaymentRequest({ transport: [], id: 'request-id-unsupported', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: unsupportedNut10 });
      const encoded = pr.toEncodedRequest();
    ...
    it('keeps malformed P2PK requirements as diagnostics and returns no payable mints', async () => {
      ...
      const pr = new PaymentRequest({ transport: [], id: 'request-id-malformed', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: malformedNut10 });
      const encoded = pr.toEncodedRequest();
    ...
    it('should decode a Nostr payment request for plugin delivery', async () => {
      const pr = new PaymentRequest({ transport: [{ type: PaymentRequestTransportType.NOSTR, target: 'npub123...' }], id: 'request-id-4', amount: 100, unit: 'sat' });
      const encoded = pr.toEncodedRequest();
    ...
    it('should return an empty payable mint list if no matching mints are found', async () => {
      ...
      const pr = new PaymentRequest({ transport: [], id: 'request-id-6', amount: 100, unit: 'sat', mints: [testMintUrl] });
      const encoded = pr.toEncodedRequest();
    ...
    it('matches custom-unit payment requests against balances for that unit only', async () => {
      const pr = new PaymentRequest({ transport: [], id: 'request-id-usd', amount: 100, unit: 'USD', mints: [testMintUrl] });
      const encoded = pr.toEncodedRequest();
  ...
  describe('prepare', () => {
    ...
    it('recomputes P2PK-aware payable mints when preparing amountless requests', async () => {
      const paymentRequest = new PaymentRequest({ transport: [], id: 'request-id-amountless-p2pk', unit: 'sat', mints: [testMintUrl, testMintUrl2], singleUse: false, nut10: p2pkNut10() });
    ...
    it('initializes P2PK send operations for valid P2PK payment requests', async () => {
      const paymentRequest = new PaymentRequest({ transport: [], id: 'request-id-prepare-p2pk', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: p2pkNut10() });
      const request: ResolvedPaymentRequest = {
        paymentRequest,
    ...
    it('re-derives P2PK requirements during prepare when the cached condition is missing', async () => {
      const paymentRequest = new PaymentRequest({ transport: [], id: 'request-id-prepare-p2pk-missing-condition', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: p2pkNut10() });
      const request: ResolvedPaymentRequest = {
        paymentRequest,
        payableMints: [testMintUrl],
        allowedMints: [testMintUrl],
    ...
    it('rejects unsupported NUT-10 requirements before initializing a send operation', async () => {
      ...
      const request: ResolvedPaymentRequest = {
        paymentRequest: new PaymentRequest({ transport: [], id: 'request-id-unsupported-prepare', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: unsupportedNut10 }),
        payableMints: [],
        allowedMints: [testMintUrl],
        amount: Amount.from(100),
        unit: 'sat',
    ...
    it('rejects HTLC requirements before initializing a send operation', async () => {
      ...
      const request: ResolvedPaymentRequest = {
        paymentRequest: new PaymentRequest({ transport: [], id: 'request-id-htlc-prepare', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: htlcNut10 }),
        payableMints: [],
        allowedMints: [testMintUrl],
        amount: Amount.from(100),
        unit: 'sat',
    ...
    it('rejects malformed P2PK requirements before initializing a send operation', async () => {
      const malformedNut10 = p2pkNut10({ data: '' });
      const request: ResolvedPaymentRequest = {
        paymentRequest: new PaymentRequest({ transport: [], id: 'request-id-malformed-prepare', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: malformedNut10 }),
        payableMints: [],
        allowedMints: [testMintUrl],
        amount: Amount.from(100),
        unit: 'sat',
    ...
    it('wraps malformed P2PK helper failures as PaymentRequestError causes during prepare', async () => {
      const malformedNut10 = p2pkNut10({ data: '' });
      const request: ResolvedPaymentRequest = {
        paymentRequest: new PaymentRequest({ transport: [], id: 'request-id-malformed-helper', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: malformedNut10 }),
        payableMints: [testMintUrl],
        allowedMints: [testMintUrl],
        amount: Amount.from(100),
        unit: 'sat',
    ...
    it('rejects selected mints without NUT-11 support before initializing a send operation', async () => {
      ...
      const paymentRequest = new PaymentRequest({ transport: [], id: 'request-id-p2pk-mint-unsupported', amount: 100, unit: 'sat', mints: [testMintUrl], singleUse: false, nut10: p2pkNut10() });
      const request: ResolvedPaymentRequest = {
        paymentRequest,
        payableMints: [testMintUrl],
        allowedMints: [testMintUrl],
