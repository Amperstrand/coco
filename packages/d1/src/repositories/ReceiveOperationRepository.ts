...
export class D1ReceiveOperationRepository implements ReceiveOperationRepository {
  ...
  async getByPaymentRequestAttemptId(attemptId: string): Promise<ReceiveOperation | null> {
    ...
    const operation = rows
      ...
      .find(
        (candidate) =>
          candidate.source?.type === 'payment-request' &&
          candidate.source.attemptId === attemptId,
      );
