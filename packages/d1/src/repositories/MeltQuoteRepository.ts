...
function rowToQuote(row: MeltQuoteRow): MeltQuote {
  ...
  if (row.method === 'onchain') {
    ...
    return {
      ...base,
      method: 'onchain',
      fee_options: feeOptions.map((option: { fee_reserve: string | number; fee_index: number; estimated_blocks: number } & Record<string, unknown>) => ({
        ...option,
...
export class D1MeltQuoteRepository implements MeltQuoteRepository {
